...
-   All channels mentioned are just for <span style={{color:'#1A6D34'}}>@Students</span> (which is 40% of the entire server).
...
1. recommended: Stackoverflow-like resources for <span style={{color:'#1A6D34'}}>@Students</span>.
2. recommended-events: Suggested events that are not organized by ACU (the Miscellaneous events in MidMap above).

## Q&A

Topics that are not studying material but related to it.
...
3. internships: Share and discuss internship options, application process, and tips between <span style={{color:'#1A6D34'}}>@Students</span>.
...
Old channels replaced with way/channels of doing things or that does not have traffic anymore (E.g., gsoc is seasonal)
